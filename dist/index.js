...
const suiClient = new client_1.SuiClient({ url: SUI_RPC_URL });
// Track active subscriptions in memory
const activeSubscriptions = new Map();
// API endpoint to get event types for a package
app.get('/api/package-events/:packageId', async (req, res) => {
...
app.get('/api/events/:eventType', async (req, res) => {
    ...
    const sinceTimestamp = req.query.since ? parseInt(req.query.since) : undefined;
    if (!eventType) {
        return res.status(400).json({ error: 'Event type is required' });
    }
    try {
        const events = await (0, suiUtils_1.getEventsForType)(suiClient, eventType, sinceTimestamp);
        res.json(events);
    }
    catch (error) {
        ...
        try {
            ...
            if (parsedMessage.type === 'subscribe') {
                ...
                try {
                    ...
                    const intervalId = setInterval(async () => {
                        try {
                            ...
                            const events = await (0, suiUtils_1.getEventsForType)(suiClient, eventType, subscription.lastPolled);
                ...
                if (ws.clientSubscriptions && ws.clientSubscriptions.has(subscriptionId)) {
                    ...
                    const subscription = ws.clientSubscriptions.get(subscriptionId);
                    try {
                        const events = await (0, suiUtils_1.getEventsForType)(suiClient, subscription.eventType, subscription.lastPolled);
                        // Update last polled timestamp
                        subscription.lastPolled = Date.now();
                        // Send events to client
                        if (events.length > 0) {
